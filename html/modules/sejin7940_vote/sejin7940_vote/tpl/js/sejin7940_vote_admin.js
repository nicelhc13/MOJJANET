function insertSelectedModule(id, module_srl, mid, browser_title) {
	...
	if(id=='target_module2') {  // 수정
	    doGetCategoryFromModule2(module_srl);
	}
	else {
...
function doGetCategoryFromModule(module_srl,category_srl) {
    var params = new Array();
    params['module_srl'] = module_srl;

    var response_tags = ['error','message','categories'];
	response_tags['category_srl'] = category_srl;
...
function completeGetCategoryFromModules(ret_obj, response_tags) {
    ...
    for(var i=0;i<category_list.length;i++) {
		...
		if(category_srl == response_tags['category_srl']) {
	        var opt = new Option(category_title, category_srl, true, true);
		}
		else {
...
/* 선택된 모듈의 카테고리 목록을 가져오는 함수 */
function doGetCategoryFromModule2(module_srl,category_srl) {
    var params = new Array();
    params['module_srl'] = module_srl;

    var response_tags = ['error','message','categories'];
	response_tags['category_srl'] = category_srl;
    exec_xml('document','getDocumentCategories',params, completeGetCategoryFromModules2, response_tags);
}

function completeGetCategoryFromModules2(ret_obj, response_tags) {
    var obj = jQuery('#target_category2').get(0);
    var length = obj.options.length;
    for(var i=0;i<length;i++) obj.remove(0);
...
function doCancelDeclare() {
    ...
    exec_xml('sejin7940_vote','procSejin7940_voteCancelDeclare', params, function() { location.reload(); });
